...
def HohmannTransfer(r_i,r_f):
    #  Only circular final and intial orbits
    mu = 398600.4418
    a_trans = (r_i + r_f)/2
    v_i = m.sqrt(mu/r_i)
    v_f = m.sqrt(mu/r_f)
    ...
    delta_v_b = v_f - v_trans_b
    delta_v = abs(delta_v_a) + abs(delta_v_b)
    period_trans = m.pi * m.sqrt((a_trans**3)/mu)
    return a_trans, period_trans, delta_v_a, delta_v_b

def BiEllipticTransfer(r_i,r_b,r_f):
    #  Only circular final and intial orbits
    mu = 398600.4418
    a_trans_1 = (r_i + r_b)/2
    a_trans_2 = (r_b + r_f)/2
    v_i = m.sqrt(mu/r_i)
    v_trans_1_b = m.sqrt(((2*mu)/r_b) - (mu/a_trans_1))
    v_trans_2_c = m.sqrt(((2*mu)/r_f) - (mu/a_trans_2))
    v_trans_1_a = m.sqrt(((2*mu)/r_i) - (mu/a_trans_1))
    v_trans_2_b = m.sqrt(((2*mu)/r_b) - (mu/a_trans_2))
    v_f = m.sqrt(mu/r_f)
    delta_v_a = v_trans_1_a - v_i
    delta_v_b = v_trans_2_b - v_trans_1_b
    delta_v_c = v_f - v_trans_2_c
    delta_v = abs(delta_v_a) + abs(delta_v_b) + abs(delta_v_c)
    period_trans = (m.pi*m.sqrt((a_trans_1**3)/mu)) + (m.pi*m.sqrt((a_trans_2**3)/mu))
    return a_trans_1, a_trans_2, period_trans, delta_v_a, delta_v_b, delta_v_c
